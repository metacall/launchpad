  ...
  const plans = [
    {
      ...
      features: ['Unlimited Functions', 'Unlimited Builds', 'Medium Resources'],
    },
    {
      id: Plans.Standard,
      name: 'Standard Plan',
      ...
      features: ['Unlimited Functions', 'Unlimited Builds', 'High Resources'],
    },
    {
      id: Plans.Premium,
      name: 'Premium Plan',
      ...
      <div className="w-full max-w-5xl mt-6 relative">
        ...
        <div className="text-center mb-12 px-4">
          <h2 className="text-2xl font-semibold text-slate-800 tracking-tight">
            Select one of our subscription plans
          </h2>
          <p className="text-slate-500 mt-2 text-sm font-medium">
            Scale effortlessly to millions of requests with MetaCall's FaaS.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-center items-stretch gap-6 px-4">
          {plans.map(plan => (
            <div
              key={plan.id}
              className={`border border-slate-300 bg-white flex flex-col relative overflow-hidden group transition-colors w-full md:w-1/3 ${plan.borderColor}`}
            >
              {/* Colored Traingle Ribbon */}
              <div
                className={`absolute top-0 right-0 w-16 h-16 ${plan.color} transform translate-x-1/2 -translate-y-1/2 rotate-45`}
              ></div>

              <div className="p-8 pb-6 grow">
                ...
                <p className="text-[10px] text-gray-400 font-bold tracking-widest uppercase mb-6 mt-1 text-center w-30">
                  per deploy
                </p>

                <p className="text-[13px] text-gray-600 mb-10 min-h-10 leading-relaxed">
                  {plan.description}
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white border border-slate-300 w-full max-w-lg flex flex-col overflow-hidden">
            ...
            <div className="p-6 pb-2">
              ...
              <div
                className={`w-full ${checkoutPlan.color} text-white px-4 py-3 flex justify-between items-center mb-6`}
              >
                <span className="font-medium text-[13px]">
                  Subscription €{checkoutPlan.price} / mo
                </span>
                <span className="text-[12px] font-bold">{checkoutPlan.name}</span>
              </div>
            </div>
            ...
            <div className="px-6 pb-6 overflow-y-auto max-h-[70vh]">
              ...
              <div className="flex items-center px-3 py-2.5 mb-6">
                <div className="w-6 h-4 bg-slate-200 rounded flex items-center justify-center mr-3 shrink-0 text-[8px] text-slate-500 font-bold border border-slate-300">
                  Card
                </div>
                <input
                  type="text"
                  placeholder="Card number"
                  className="grow outline-none text-[13px] placeholder:text-slate-400"
                />
